...
**Weights** have a range from -1.00 to 1.00. -1 means that the inputs decrease the probability of the the thing we are trying to predict and 1 means it acts increases the probability.
...
Lest's say we guessed the value of Y1 but it's wrong and we know the value of an error. Let's call this error EY1. What should be the error of X1 or X2? One might say we should split the error of Y1 evenly because there are two nodes connected to Y1, so the error of X1 should be 1/2 of error of Y1 and the error of X2 should be 1/2 of the error of Y1. Well it's generally not a good way of doing this, beause we can see that weight coming from X2 is 3 times larger than the one of of X1. Because of that X2 contributes to the mistake of Y1 3 times as much as X1. Therefore it would be better to describe the error of X1 as 1/4 of the error of Y1 and the error of X2 as 3/4 of the error of Y1. This can lead to a general principle:
<br/>
...
Now we can see that the weight of a matrix is the same as the one we used to feed forward values between the layers, ecept now it's transposed, which means it is flipped along the diagonal line. And if you think about it, it makes a perfect sence, now we are going backwards, instead of forward, which means we need to invert the matrix:
